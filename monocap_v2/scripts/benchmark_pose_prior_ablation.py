...
DEFAULT_VARIANTS = "baseline,conservative,knee90,knee105,lower_limb_walk,lower_limb_walk_limited"
...
CSV_FIELDS = [
    ...
    "passes",
    "total_corrections",
    "left_knee_violations_before",
    "left_knee_violations_after",
    "right_knee_violations_before",
    "right_knee_violations_after",
    "left_ankle_violations_before",
    "left_ankle_violations_after",
    "right_ankle_violations_before",
    "right_ankle_violations_after",
    "mean_joint_displacement_m",
    "max_joint_displacement_m",
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Ablate geometric pose-prior joint limits against Level A reports.")
...
def main() -> int:
    ...
    out_dir = args.out or (
        repo_root / "monocap_v2" / "benchmarks" / f"{manifest.get('subject_id', 'subject')}_{args.activity}_pose_prior"
    )
    ...
    for trial in trials:
        ...
        for backend in backends:
            ...
            for variant in variants:
                try:
                    corrected = pose
                    prior_report: dict[str, Any] = {"status": "baseline", "mocap_used_in_objective": False}
                    if variant != "baseline":
                        prior_cfg = _variant_cfg(variant, run_config)
                        corrected, prior_report = apply_pose_prior_to_pose(pose, prior_cfg)
                        if prior_report.get("status") != "ok":
                            row = failure_row(
                                backend,
                                trial,
                                run_dir,
                                "skipped",
                                prior_report.get("reason", "Pose-prior refinement skipped."),
                            )
                            row.update(_prior_row_fields(variant, prior_report))
                            rows.append(row)
                            continue
                    ...
                    row.update(_prior_row_fields(variant, prior_report))
                    row["warnings"] = _merge_warnings(row.get("warnings"), prior_report.get("warnings"))
                    rows.append(row)
                except Exception as exc:
...
def write_outputs(out_dir: Path, rows: list[dict[str, Any]], args: argparse.Namespace, manifest: dict[str, Any]) -> dict[str, str]:
    csv_path = out_dir / "pose_prior_ablation.csv"
    ...
    json_path = out_dir / "pose_prior_ablation.json"
    write_json(json_path, summary)
    md_path = out_dir / "pose_prior_ablation.md"
...
def _variant_cfg(variant: str, run_config: dict[str, Any] | None = None) -> dict[str, Any]:
    base = (((run_config or {}).get("config") or {}).get("optimization") or {}).get("pose_prior") or {}
    cfg = dict(base)
    cfg["enabled"] = True
    cfg.setdefault("blend", 1.0)
    cfg.setdefault("passes", 1)
    cfg.setdefault("max_joint_displacement_m", 0.20)
    name = variant.strip().lower()
    if name == "baseline":
        cfg["enabled"] = False
    elif name in {"conservative", "lower_limb_conservative"}:
        cfg["limit_set"] = "lower_limb_conservative"
    elif name == "knee90":
        cfg["limit_set"] = "walking_knee90"
    elif name == "knee105":
        cfg["limit_set"] = "walking_knee105"
    elif name in {"lower_limb_walk", "walking_lower_limb"}:
        cfg["limit_set"] = "walking_lower_limb"
    elif name == "lower_limb_walk_limited":
        cfg["limit_set"] = "walking_lower_limb"
        cfg["max_joint_displacement_m"] = 0.10
    else:
        raise ValueError(f"Unsupported pose-prior variant: {variant}")
    return cfg


def _prior_row_fields(variant: str, prior_report: dict[str, Any]) -> dict[str, Any]:
    before = prior_report.get("angle_metrics_before") or {}
    after = prior_report.get("angle_metrics_after") or {}
    metrics_after = prior_report.get("metrics_after") or {}
    return {
        "variant": variant,
        "method": prior_report.get("method"),
        "limit_set": prior_report.get("limit_set"),
        "blend": prior_report.get("blend"),
        "passes": prior_report.get("passes"),
        "total_corrections": prior_report.get("total_corrections"),
        "left_knee_violations_before": _violations(before, "left_knee"),
        "left_knee_violations_after": _violations(after, "left_knee"),
        "right_knee_violations_before": _violations(before, "right_knee"),
        "right_knee_violations_after": _violations(after, "right_knee"),
        "left_ankle_violations_before": _violations(before, "left_ankle"),
        "left_ankle_violations_after": _violations(after, "left_ankle"),
        "right_ankle_violations_before": _violations(before, "right_ankle"),
        "right_ankle_violations_after": _violations(after, "right_ankle"),
        "mean_joint_displacement_m": metrics_after.get("mean_joint_displacement_m"),
        "max_joint_displacement_m": metrics_after.get("max_joint_displacement_m"),
        "median_bone_length_error_after_mm": metrics_after.get("median_bone_length_error_mm"),
        "max_bone_length_error_after_mm": metrics_after.get("max_bone_length_error_mm"),
    }


def _violations(metrics: dict[str, Any], joint: str) -> Any:
    return (metrics.get(joint) or {}).get("violation_frames")
        ...
        out.append(
            {
                ...
                "total_corrections": best.get("total_corrections"),
                "max_joint_displacement_m": best.get("max_joint_displacement_m"),
            }
        )
    return out
...
def _write_markdown(path: Path, summary: dict[str, Any]) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write("# monocap_v2 Pose-Prior / Joint-Limit Ablation\n\n")
        f.write("- Geometric joint-angle limits are applied to joints-only artifacts.\n")
        f.write("- Mocap/OpenSim FK is reporting-only and is not used to choose corrections.\n\n")
        f.write("## Best By Backend/Trial\n\n")
        f.write("| Backend | Trial | Best Variant | Baseline MPJPE | Best MPJPE | Improvement | Corrections | Max Disp m |\n")
        f.write("|---|---|---|---:|---:|---:|---:|---:|\n")
        for item in summary.get("best_by_backend_trial") or []:
            f.write(
                ...
                f"{_fmt(item.get('improvement_vs_baseline_mm'))} | {_fmt(item.get('total_corrections'))} | "
                f"{_fmt(item.get('max_joint_displacement_m'))} |\n"
            )
        f.write("\n## Rows\n\n")
        f.write("| Backend | Trial | Variant | Status | Primary | Rigid | PA | Corrections | LK Before/After | RK Before/After | LA Before/After | RA Before/After | Max Disp | Warnings | Error |\n")
        f.write("|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n")
        for row in rows:
            f.write(
                ...
                f"{_fmt(row.get('pa_mpjpe_mm'))} | {_fmt(row.get('total_corrections'))} | "
                f"{_fmt(row.get('left_knee_violations_before'))}/{_fmt(row.get('left_knee_violations_after'))} | "
                f"{_fmt(row.get('right_knee_violations_before'))}/{_fmt(row.get('right_knee_violations_after'))} | "
                f"{_fmt(row.get('left_ankle_violations_before'))}/{_fmt(row.get('left_ankle_violations_after'))} | "
                f"{_fmt(row.get('right_ankle_violations_before'))}/{_fmt(row.get('right_ankle_violations_after'))} | "
                f"{_fmt(row.get('max_joint_displacement_m'))} | {_warning_text(row.get('warnings'))} | {row.get('error') or ''} |\n"
            )
...
def _print_summary(outputs: dict[str, str], rows: list[dict[str, Any]]) -> None:
    ...
    print(f"[pose-prior] Valid rows: {valid}/{len(rows)}", flush=True)
    print(f"[pose-prior] Summary: {outputs['markdown']}", flush=True)
    print(f"[pose-prior] CSV: {outputs['csv']}", flush=True)
