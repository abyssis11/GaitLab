...
DEFAULT_VARIANTS = "baseline,kc_ma5,kc_ma9,kc_ma9_noroot,kc_savgol9,kc_ma9_blend50"
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Ablate fixed-length kinematic-chain refinement against Level A reports.")
...
def main() -> int:
    ...
    out_dir = args.out or (
        repo_root / "monocap_v2" / "benchmarks" / f"{manifest.get('subject_id', 'subject')}_{args.activity}_kinematic_chain"
    )
    ...
    for trial in trials:
        ...
        for backend in backends:
            ...
            for variant in variants:
                try:
                    corrected = pose
                    chain_report: dict[str, Any] = {"status": "baseline", "mocap_used_in_objective": False}
                    if variant != "baseline":
                        chain_cfg = _variant_cfg(variant, run_config)
                        corrected, chain_report = apply_kinematic_chain_to_pose(pose, chain_cfg)
                        if chain_report.get("status") != "ok":
                            row = failure_row(
                                backend,
                                trial,
                                run_dir,
                                "skipped",
                                chain_report.get("reason", "Kinematic-chain refinement skipped."),
                            )
                            row.update(_chain_row_fields(variant, chain_report))
                            rows.append(row)
                            continue
...
def write_outputs(out_dir: Path, rows: list[dict[str, Any]], args: argparse.Namespace, manifest: dict[str, Any]) -> dict[str, str]:
    csv_path = out_dir / "kinematic_chain_ablation.csv"
    ...
    json_path = out_dir / "kinematic_chain_ablation.json"
    write_json(json_path, summary)
    md_path = out_dir / "kinematic_chain_ablation.md"
...
def _variant_cfg(variant: str, run_config: dict[str, Any] | None = None) -> dict[str, Any]:
    base = (
        (((run_config or {}).get("config") or {}).get("optimization") or {}).get("kinematic_chain") or {}
    )
    cfg = dict(base)
    cfg["enabled"] = True
    ...
    if name == "baseline":
        cfg["enabled"] = False
        return cfg
    if name.startswith("kc_ma"):
        cfg["method"] = "moving_average"
        suffix = name[5:]
        window_text = suffix.split("_", 1)[0]
        cfg["window_frames"] = int(window_text)
    elif name.startswith("kc_savgol"):
        cfg["method"] = "savgol"
        suffix = name[9:]
        window_text = suffix.split("_", 1)[0]
        cfg["window_frames"] = int(window_text)
    else:
        raise ValueError(f"Unsupported kinematic-chain variant: {variant}")
    cfg["direction_window_frames"] = int(cfg["window_frames"])
    cfg["root_window_frames"] = int(cfg["window_frames"])
    cfg["smooth_roots"] = not name.endswith("_noroot")
    cfg["direction_blend"] = 0.5 if name.endswith("_blend50") else 1.0
    cfg["output_blend"] = 0.5 if name.endswith("_blend50") else 1.0
    cfg.setdefault("max_joint_displacement_m", 0.20)
    return cfg


def _chain_row_fields(variant: str, chain_report: dict[str, Any]) -> dict[str, Any]:
    before = chain_report.get("metrics_before") or {}
    after = chain_report.get("metrics_after") or {}
    return {
        "variant": variant,
        "method": chain_report.get("method"),
        "smoothing_method": chain_report.get("smoothing_method"),
        "window_frames": chain_report.get("window_frames"),
        "direction_window_frames": chain_report.get("direction_window_frames"),
        "root_window_frames": chain_report.get("root_window_frames"),
        "smooth_roots": chain_report.get("smooth_roots"),
        "direction_blend": chain_report.get("direction_blend"),
        "output_blend": chain_report.get("output_blend"),
        "edge_count": chain_report.get("edge_count"),
        ...
        "mean_joint_displacement_m": after.get("mean_joint_displacement_m"),
        "max_joint_displacement_m": after.get("max_joint_displacement_m"),
    }
...
def _write_markdown(path: Path, summary: dict[str, Any]) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write("# monocap_v2 Kinematic-Chain Ablation\n\n")
        f.write("- Kinematic-chain refinement smooths root joints and fixed-length segment directions.\n")
        f.write("- Mocap/OpenSim FK is reporting-only and is not used to choose chain parameters or outputs.\n\n")
        ...
        f.write("\n## Rows\n\n")
        f.write("| Backend | Trial | Variant | Status | Primary | Rigid | PA | Smooth After | Bone Error | Joint Disp | Error |\n")
        f.write("|---|---|---|---|---:|---:|---:|---:|---:|---:|---|\n")
        for row in rows:
            f.write(
                ...
                f"{_fmt(row.get('pa_mpjpe_mm'))} | {_fmt(row.get('mean_second_diff_after_m'))} | "
                f"{_fmt(row.get('median_bone_length_error_after_mm'))} | {_fmt(row.get('mean_joint_displacement_m'))} | "
                f"{row.get('error') or ''} |\n"
            )
...
def _print_summary(outputs: dict[str, str], rows: list[dict[str, Any]]) -> None:
    ...
    print(f"[kinematic-chain] Valid rows: {valid}/{len(rows)}", flush=True)
    print(f"[kinematic-chain] Summary: {outputs['markdown']}", flush=True)
    print(f"[kinematic-chain] CSV: {outputs['csv']}", flush=True)
