...
from monocap_v2.scripts import benchmark_subject_scale_ablation as script


def test_subject_scale_ablation_runs_on_fake_cached_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "benchmark_subject_scale_ablation.py",
            "--manifest",
            str(manifest_path),
            "--paths",
            str(paths_path),
    ...
    assert (tmp_path / "out" / "subject_scale_ablation.csv").exists()
    summary = script.read_json(Path(tmp_path / "out" / "subject_scale_ablation.json")) if hasattr(script, "read_json") else None
    if summary is None:
        import json

        summary = json.loads((tmp_path / "out" / "subject_scale_ablation.json").read_text(encoding="utf-8"))
    variants = {row["variant"] for row in summary["rows"]}
    assert variants == {"baseline", "height_global"}
...
def _write_pose_run(run_dir: Path) -> None:
    ...
    write_yaml(run_dir / "run_config.yaml", {"config": {"optimization": {"subject_scale": {"min_scale": 0.5, "max_scale": 3.0}}}})
