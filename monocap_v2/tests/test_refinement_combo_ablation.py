...
from monocap_v2.scripts import benchmark_refinement_combo_ablation as script


def test_refinement_combo_ablation_runs_on_fake_cached_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "benchmark_refinement_combo_ablation.py",
            "--manifest",
            str(manifest_path),
            "--paths",
            str(paths_path),
            ...
            "metrabs",
            "--variants",
            "baseline,best_scale_ma9_bone_root_toe",
            "--scale-modes",
            "metrabs=height_bone",
            "--repo-root",
            str(tmp_path),
    ...
    assert (tmp_path / "out" / "refinement_combo_ablation.csv").exists()
    summary = json.loads((tmp_path / "out" / "refinement_combo_ablation.json").read_text(encoding="utf-8"))
    variants = {row["variant"] for row in summary["rows"]}
    assert variants == {"baseline", "best_scale_ma9_bone_root_toe"}
...
def _write_pose_run(run_dir: Path) -> None:
    ...
    with (run_dir / "pose3d_initial" / "pose3d_initial.pkl").open("wb") as f:
        pickle.dump(pose, f)
    (run_dir / "contacts").mkdir()
    np.savez_compressed(
        run_dir / "contacts" / "contacts.npz",
        left_heel=np.zeros(6, dtype=np.float32),
        left_toe=np.ones(6, dtype=np.float32),
        right_heel=np.zeros(6, dtype=np.float32),
        right_toe=np.zeros(6, dtype=np.float32),
        backend="test",
        activity="walking",
    )
    (run_dir / "input").mkdir()
    write_json(run_dir / "input" / "subject.json", {"height_m": 1.8})
    write_yaml(
        run_dir / "run_config.yaml",
        {
            "config": {
                "optimization": {
                    "subject_scale": {"min_scale": 0.5, "max_scale": 3.0},
                    "temporal_smoothing": {"max_joint_displacement_m": 0.15},
                    "contact_foot_locking": {"max_correction_m": 0.2},
                }
            }
        },
    )
