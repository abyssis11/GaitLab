...
from monocap_v2.scripts import benchmark_temporal_smoothing_ablation as script


def test_temporal_smoothing_ablation_runs_on_fake_cached_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "benchmark_temporal_smoothing_ablation.py",
            "--manifest",
            str(manifest_path),
            "--paths",
            str(paths_path),
    ...
    assert (tmp_path / "out" / "temporal_smoothing_ablation.csv").exists()
    summary = json.loads((tmp_path / "out" / "temporal_smoothing_ablation.json").read_text(encoding="utf-8"))
    variants = {row["variant"] for row in summary["rows"]}
    assert variants == {"baseline", "ma3_bone"}
...
def _write_pose_run(run_dir: Path) -> None:
    ...
    with (run_dir / "pose3d_initial" / "pose3d_initial.pkl").open("wb") as f:
        pickle.dump(pose, f)
    write_yaml(run_dir / "run_config.yaml", {"config": {"optimization": {"temporal_smoothing": {"max_joint_displacement_m": 0.15}}}})
