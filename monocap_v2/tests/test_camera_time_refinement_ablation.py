...
from monocap_v2.scripts import benchmark_camera_time_refinement_ablation as script


def test_camera_time_refinement_ablation_runs_on_fake_cached_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "benchmark_camera_time_refinement_ablation.py",
            "--manifest",
            str(manifest_path),
            "--paths",
            str(paths_path),
    ...
    assert (tmp_path / "out" / "camera_time_refinement_ablation.csv").exists()
    summary = json.loads((tmp_path / "out" / "camera_time_refinement_ablation.json").read_text(encoding="utf-8"))
    variants = {row["variant"] for row in summary["rows"]}
    assert variants == {"baseline", "time_only"}
    assert all(row["status"] == "valid" for row in summary["rows"])
    time_only = next(row for row in summary["rows"] if row["variant"] == "time_only")
    assert float(time_only["selected_time_offset_ms"]) != 0.0
            ...
            "config": {
                "optimization": {
                    "camera_time_refinement": {
                        "time_search": {"min_offset_s": -0.05, "max_offset_s": 0.05, "step_s": 1.0 / 60.0},
                        "camera_delta": {"max_nfev": 5},
                    }
                }
            }
        },
...
def _write_reference(tmp_path: Path) -> tuple[Path, Path]:
    ...
    np.savez(path, time_s=np.arange(10, dtype=float) / 60.0, joints_m=_reference_joints(), joint_names=names)
...
def _reference_joints() -> np.ndarray:
    joints = np.zeros((10, 7, 3), dtype=float)
    joints[:, :, 2] = 3.0
    ...
    joints[:, :, 0] += np.linspace(0.0, 0.1, 10)[:, None]
    return joints
