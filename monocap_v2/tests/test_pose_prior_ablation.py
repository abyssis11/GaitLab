...
from monocap_v2.scripts import benchmark_pose_prior_ablation as script


def test_pose_prior_ablation_runs_on_fake_cached_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "benchmark_pose_prior_ablation.py",
            "--manifest",
            str(manifest_path),
            "--paths",
            str(paths_path),
    ...
    assert (tmp_path / "out" / "pose_prior_ablation.csv").exists()
    summary = json.loads((tmp_path / "out" / "pose_prior_ablation.json").read_text(encoding="utf-8"))
    variants = {row["variant"] for row in summary["rows"]}
    assert variants == {"baseline", "knee90"}
    assert all(row["status"] == "valid" for row in summary["rows"])
    knee90 = next(row for row in summary["rows"] if row["variant"] == "knee90")
    assert knee90["total_corrections"] > 0
...
def _write_pose_run(run_dir: Path) -> None:
    ...
    joints = _reference_joints()
    camera_like = joints.copy()
    camera_like[:, :, 1] *= -1.0
    camera_like[:, names.index("left_ankle"), :] = camera_like[:, names.index("left_knee"), :] + [0.05, -0.05, 0.0]
    pose = {
    ...
    with (run_dir / "pose3d_initial" / "pose3d_initial.pkl").open("wb") as f:
        pickle.dump(pose, f)
    write_yaml(run_dir / "run_config.yaml", {"config": {"optimization": {"pose_prior": {"max_joint_displacement_m": 0.2}}}})
...
def _write_reference(tmp_path: Path) -> tuple[Path, Path]:
    ...
    np.savez(path, time_s=np.arange(8, dtype=float) / 60.0, joints_m=_reference_joints(), joint_names=names)
...
def _reference_joints() -> np.ndarray:
    joints = np.zeros((8, 7, 3), dtype=float)
    joints[:, :, 2] = 3.0
    ...
    joints[:, :, 0] += np.linspace(0.0, 0.1, 8)[:, None]
    return joints
