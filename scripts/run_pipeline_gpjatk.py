...
def main():
    MANIFEST = "./manifests/GPJATK/subject7.yaml"
    CONFIG = "./config/paths.yaml"
    TRIAL = "walking1"

    # Commands as provided (kept verbatim on purpose).
    steps = [
        f"python src/utils/gpjatk_mocap_to_trc.py -m {MANIFEST} -p {CONFIG} --trial {TRIAL}",
        ...
        f"python src/validation/validate_rtmw3d_vs_mocap_gpjatk.py  -m {MANIFEST} -p {CONFIG} --trial {TRIAL} --root-center pelvis --summary-per-joint --procrustes similarity --resample-to mocap --out-json"
    ]
